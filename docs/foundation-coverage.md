...
The checks are deterministic, read-only, and require only `filesystem-read`. Repository candidate files may contain credential-like content, and the bounded evaluator can read those bytes while assessing documentation. No credential value is requested or used to access an external service. The checks do not connect to a provider or access a deployment or production environment. The checks do not inspect live providers, deployment state, alert delivery, health endpoint responses, backup creation, restore results, or rollback execution.
