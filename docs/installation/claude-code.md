...
Before copying, remove exactly these four directories from the current project's `.claude/skills`: `post-vibe-clarity`, `project-discovery`, `secret-exposure`, and `launch-essentials`. Then, from the project root, repeat the install procedure and verify discovery again.
