...
In this project, confirm Claude Code discovers `post-vibe-clarity`, `project-discovery`, `secret-exposure`, `launch-essentials`, `launch-operations`, and `universal-verification`. Invoke `/post-vibe-clarity`. If Claude Code does not discover a newly created top-level skill directory, restart Claude Code and verify again. Confirm `.claude/skills/.postvibeclarity-revision` records the expected tag and commit.
