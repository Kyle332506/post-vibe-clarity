...
In this project, confirm Cursor discovers `post-vibe-clarity`, `project-discovery`, `secret-exposure`, and `launch-essentials`. Invoke `/post-vibe-clarity`. If Cursor does not discover a newly created top-level skill directory, restart Cursor and verify again.
