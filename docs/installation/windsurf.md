...
In this project, confirm Windsurf discovers `post-vibe-clarity`, `project-discovery`, `secret-exposure`, `launch-essentials`, `launch-operations`, and `universal-verification`. Invoke `@post-vibe-clarity`. If Windsurf does not discover a newly created top-level skill directory, restart Windsurf and verify again. Confirm `.agents/skills/.postvibeclarity-revision` records the expected tag and commit.
