...
In this project, confirm Codex discovers `post-vibe-clarity`, `project-discovery`, `secret-exposure`, and `launch-essentials`. Invoke `$post-vibe-clarity`. If Codex does not discover a newly created top-level skill directory, restart Codex and verify again.
