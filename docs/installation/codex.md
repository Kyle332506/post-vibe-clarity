# Install PostVibeClarity with Codex

Install these skills only in the current project's `.agents/skills` directory.

## Install

From the project root, clone the reviewed release tag, stage all six skills, compare any existing copies, preserve them in a bounded backup, and record the exact installed commit:
...
The block exits on the first failed command and validates all six staged directories before moving any live skill. The `diff -qr` preflight reports changed paths without printing file contents. Existing destinations are moved, not deleted, and remain under the unique `PVC_BACKUP_ROOT`. Keep that backup until verification succeeds. You can then remove the explicit temporary directory represented by `PVC_TEMP_ROOT` and, after reviewing it, the specific backup directory printed in your shell state; do not use a broad or unresolved removal command.

## Verify

In this project, confirm Codex discovers `post-vibe-clarity`, `project-discovery`, `secret-exposure`, `launch-essentials`, `launch-operations`, and `universal-verification`. Invoke `$post-vibe-clarity`. If Codex does not discover a newly created top-level skill directory, restart Codex and verify again. Confirm `.agents/skills/.postvibeclarity-revision` records the expected tag and commit.
...
Before uninstalling, compare the six installed directories with the revision recorded in `.agents/skills/.postvibeclarity-revision`. Move any locally changed `post-vibe-clarity`, `project-discovery`, `secret-exposure`, `launch-essentials`, `launch-operations`, or `universal-verification` directory into a named project backup before removing only those six bounded destinations and the revision record.

## Compatibility evidence

Codex's skill documentation is the [compatibility evidence](https://developers.openai.com/codex/skills) for this entry.
